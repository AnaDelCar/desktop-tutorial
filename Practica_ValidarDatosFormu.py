...
def guardarDatos():
    ...
    #VALIDAR QUE LOS CAMPOS TENGAN LOS FORMATOS CORRECTOS
    if (esEnteroValido(edad) and esDecimalValido(estatura) and esEnteroValido10Digitos(telefono) and esTextoValido(nombre) and esTextoValido(apellido)):
...
#CREAR UNA CADENA CON LOS DATOS
        ...
        messagebox.showerror("ERROR","Porfavor, Ingrese datos validos")
def limpiarCampos():
    entry_Nombre.delete(0,tk.END)
    entry_Apellido.delete(0,tk.END)
    entry_Edad.delete(0,tk.END)
...
def esEnteroValido(valor):
    try:
        int(valor)
        return True
    except ValueError:
        return False
def esDecimalValido(valor):
    try:
        float(valor)
        return True
    except ValueError:
        return False
def esEnteroValido10Digitos(valor):
